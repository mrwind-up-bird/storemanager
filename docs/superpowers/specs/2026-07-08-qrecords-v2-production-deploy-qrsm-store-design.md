...
| **No wildcard cert** | One `*.qrsm.store` SAN cert covers every subdomain (§4.2, §2 Goals, §13, §11 line 220) | **Per-host** TLS-ALPN-01 certs, one per enumerated Host() (`admin.`, `demo.`, `qrsm.store`, `www.`) | No wildcard order will ever appear in Traefik logs. Verify per-host certs. Adding a tenant = add its Host() + redeploy (DNS already resolves via the wildcard `A *`; no per-tenant DNS change). |
...
**Net effect for operators:** use `/opt/storemanager`; DNS is at IONOS; certs are per-host TLS-ALPN-01 (no wildcard, no Hetzner token, no DNS-01); routing is enumerated `Host()` (add tenant = add Host() + redeploy; DNS wildcard `A *` already resolves every subdomain); credential mail comes **from** `noreply@nyxcore.cloud`; the deploy pulls `ghcr.io/mrwind-up-bird/storemanager` using the CI `GITHUB_TOKEN` (no PAT on the host); billing runs the `fake` driver. The companion `docs/RUNBOOK-qrsm-store.md` carries the same stale claims (Hetzner DNS, `/opt/q-records-storemanager`, wildcard cert, `noreply@qrsm.store`, `GHCR_PULL_TOKEN`) and should be read through this same lens until corrected.
